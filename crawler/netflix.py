...
description_base_url = 'https://jobs.netflix.com'

# Base url of job directory page (where jobs are listed)
listing_base_url = 'https://jobs.netflix.com/search?team=Data%20Science%20and%20Engineering'
...
def scrape_dir_page(driver, num_pages = 5):
    ...
    for page_number in range(1, num_pages):
        ...
        for link_holder in soup.find_all('a',class_='css-2y5mtm essqqm81'): #get list of all <div> of class 'name'
...
def scrape_job_page(job_description_url, driver):
    ...
    job_description_soup = remove_script(job_description_soup).find('section', id='job-content')
    job_description = ''
