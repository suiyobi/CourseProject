...
description_base_url = 'https://careers.airbnb.com'

# Base url of job directory page (where jobs are listed)
listing_base_url = 'https://careers.airbnb.com/positions/?page='
...
def scrape_dir_page(driver, num_pages = 2):
    ...
    for page_number in range(1, num_pages):
        ...
        for link_holder in soup.find_all('a',class_='jobs-board__positions__list__item__link'): #get list of all <div> of class 'name'
            rel_link = link_holder['href'] #get url
            #url returned is relative, so we need to add base url
            # job_links.append(description_base_url + rel_link)
            job_links.append(rel_link)
...
def scrape_job_page(job_description_url, driver):
    ...
    job_description_soup = remove_script(job_description_soup).find('div', class_='page-positions__overview active')
    job_description = ''
