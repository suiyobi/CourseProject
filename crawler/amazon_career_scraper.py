...
description_base_url = 'https://www.amazon.jobs'

# Base url of job directory page (where jobs are listed)
listing_base_url = 'https://www.amazon.jobs/en/job_categories/software-development/?page='
...
def scrape_dir_page(driver, num_pages = 2):
    ...
    for page_number in range(1, num_pages):
        ...
        for link_holder in soup.find_all('a',class_='job-link'): #get list of all <div> of class 'name'
...
def scrape_job_page(job_description_url, driver):
    ...
    job_description_soup = remove_script(job_description_soup).find('div', class_='section description')
    job_description = ''
