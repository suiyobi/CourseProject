...
description_base_url = 'https://www.uber.com/global/en'

# Base url of job directory page (where jobs are listed)
listing_base_url = 'https://www.uber.com/us/en/careers/list/?page='
...
"""
def scrape_dir_page(driver, num_pages = 3):
    job_links = []
    ...
    for page_number in range(1, num_pages):
        ...
        for link_holder in soup.find_all('a',class_='ka pp mm pq pr ps cc bk pt'): #get list of all <div> of class 'name'
...
def scrape_job_page(job_description_url, driver):
    ...
    job_description_soup = remove_script(job_description_soup).find('div', class_='in jg')
    job_description = ''
